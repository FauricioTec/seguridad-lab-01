...
## Empezamos a filtrar
# Permitimos el tráfico en loopback (necesario para conexiones locales)
iptables -A INPUT -i lo -j ACCEPT

# Permitimos todo el tráfico desde nuestra propia IP
iptables -A INPUT -s "$IP_MAQUINA_LOCAL" -j ACCEPT

# Permitimos a la segunda máquina acceder a MySQL
...
# Cerramos el rango de puertos privilegiados (1-1024)
iptables -A INPUT -p tcp --dport 1:1024 -j DROP
iptables -A INPUT -p udp --dport 1:1024 -j DROP

# Cerramos puertos específicos
iptables -A INPUT -p tcp --dport 3306 -j DROP  # MySQL
iptables -A INPUT -p tcp --dport 10000 -j DROP  # Webmin u otros servicios
iptables -A INPUT -p udp --dport 10000 -j DROP  # Webmin u otros servicios en UDP
